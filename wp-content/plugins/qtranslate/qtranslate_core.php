...
function qtrans_init() {
	...
	if(defined('WP_ADMIN')) {
		...
		if(isset($_GET['lang']) && qtrans_isEnabled($_GET['lang'])) {
			$q_config['language'] = $q_config['url_info']['language'];
			setcookie('qtrans_client_language', $q_config['url_info']['language'], time()+60*60*24*30);			
		}elseif(isset($_COOKIE['qtrans_client_language']) && qtrans_isEnabled($_COOKIE['qtrans_client_language'])) {
			$q_config['language'] = $_COOKIE['qtrans_client_language'];
		} else {
			$q_config['language'] = $q_config['default_language'];
		}		
	}
