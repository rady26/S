  ...
  function property_overview_image($args = '') {
    ...
    if(!empty($image)) {
      ...
      <div class="property_image">
        <a href="<?php echo $property['permalink']; ?>" title=""  class="property_overview_thumb property_overview_thumb_<?php echo $thumbnail_size; ?> <?php echo $link_class; ?> thumbnail" rel="properties" >
          <img width="<?php echo $image['width']; ?>" height="<?php echo $image['height']; ?>" src="<?php echo $image['link']; ?>" alt="<?php echo $property['post_title'];?>" style="width:<?php echo $image['width']; ?>px;height:<?php echo $image['height']; ?>px;" class="linkimg" />
        </a>
      </div>
      <?php
    ...
    if($sort_by_groups != 'true' || !is_array($groups)) {
      ...
      foreach ($stats as $label => $value) {
        ...
        switch($display) {
          ...
          case 'list':
            ?>
            <tr class="<?php echo $stats_prefix; ?>_<?php echo $tag; ?> wpp_stat_plain_list_<?php echo $tag; ?> <?php echo $alt; ?>">
              <td class="td1 attribute"><?php echo $label; ?><span class="wpp_colon">:</span></td>
...
if(!function_exists('draw_property_search_form')):
  function draw_property_search_form($args = false) {
    ...
    foreach($search_groups as $group_key => $search_attributes) {
      ...
      <?php } ?>
      <li class="wpp_search_form_element submit"><input type="submit" class="wpp_search_button submit btn btn-large" value="" /></li>
    </ul>
    </form>
  <?php }
endif;
...
function getImage() {
	...
	for($i=1;$i<=$count;$i++) {
		...
		if(stristr($postOutput,'<img')) { echo '<li>'.$postOutput."</li>"; }
		$start=$imgBeg+1;
	}
	$more = 0;
}
